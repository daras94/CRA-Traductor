...
/*****************************************************************************************/
% Traductor.
oracion(s(S,V,O))         --> g_nominal(S,N), dicES:verbo(V,N), g_nominal(O,_).
oracion(s(SN,SV))         --> g_nominal(SN,N), g_verbal(SV,N).
oracion(s(O1,CC,O2))      --> oracion(O1), dicES:conjuncion_coord(CC), oracion(O2).
oracion(o(CS,SV,SN))      --> dicES:conjuncion_sub(CS), g_verbal(SV), g_nominal(SN).


/*****************************************************************************************/
/*                                Grupos Nominales:                                      */
/*****************************************************************************************/
g_nominal(gn(S),N)     --> dicES:nombre(S,_,N).
...
g_verbal(gv(V,SN),N)    --> dicES:verbo(V, N), g_nominal(SN, N).
% g_verbal(gv(V,P,GN))    --> dicES:verbo(V, N), dicES:preposicion(P), g_nominal().
